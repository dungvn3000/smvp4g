 ...
 * @since 5/24/12, 1:24 AM
 */
@Reflection
public abstract class BasicView<C extends Constants> extends FlexTable implements View<C> {
    ...
    public String getParentDomId() {
        if (parentDomId == null) {
            ...
            ComponentView componentView = ClassUtils.getAnnotation(getClass(), ComponentView.class);
            if (view != null) {
                setParentDomId(view.parentDomId());
            } else if (componentView != null) {
                setParentDomId(componentView.parentDomId());
            }
        }
        return parentDomId;
    }
